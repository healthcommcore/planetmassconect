<?php
  ...
  $days_new = $params->get('Ldaysnew');
  $txt_new = $params->get('Ltxtnew');
  $shownew = $params->get('Lshownew');
  $showhits = $params->get('Lshowhits');
  $shownumlnk = $params->get('Lshownumlnk');
  $showcdes = $params->get('Lshowcdes');
  $showldes = $params->get('Lshowldes');
  $lengthoftitle = $params->get('Llengthoftitle');
  $dotaddlenght = $params->get('Ldotaddlenght');
  $titlepopup = $params->get('Ltitlepopuptxt');
  $lnkicon = $params->get('Licon');
  $moduleclass_sfx = $params->get('moduleclass_sfx');
  $dropdown = $params->get('Ldropdown');
  ...
  if ($Corder != $cs ) # new category
  {
    ...
    $content .= "<span class=\"resourceheader\">".htmlspecialchars($row->ctitle)."</span>";
	$content .= "\n<li>";
    ...
    if ($showcdes) {
      ...
      if ("$row->cdes"!="" && "$row->cdes"!="<br />" && "$row->cdes"!="<br>" && "$row->cdes"!="<p><br></p>" && "$row->cdes"!="<p>&#160;</p>" && "$row->cdes"!="<div>&#160;</div>")
    	$content .= "<span class=\"cdes".$moduleclass_sfx."\">".$row->cdes."</span>";
	}
